	...
	<div class="desktop:w-1/3 w-full h-full flex flex-col items-center justify-center p-6">
		...
		<div class="w-full flex -tems-center justify-center mt-5">
			<div class="flex flex-col items-center justify-center">
				<button on:click={goToEdit} class="btn btn-primary w-24">edit</button>
				<button
					on:click={goToLineup}
		...
		<div class="w-full flex flex-row items-center justify-start overflow-x-scroll p-4 scale-105">
			{#each data.artists as artist}
				<div
					class="flex flex-col shrink-0 h-48 w-32 items-center justify-start bg-zinc-900 hover:bg-zinc-950 rounded-xl hover:scale-105 transition duration-300 ease-in-out mx-1"
				>
					<img
						class="object-cover h-40 w-40 rounded-t-xl"
